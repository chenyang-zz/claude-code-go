...
// TestRunnerRunPlanCommandReportsFallback verifies /plan is routed through the shared registry and emits the stable guidance fallback.
func TestRunnerRunPlanCommandReportsFallback(t *testing.T) {
	var buf bytes.Buffer
	eng := &recordingEngine{}
	runner := NewRunner(eng, console.NewStreamRenderer(console.NewPrinter(&buf)))
	registerSlashCommands(t, runner, servicecommands.PlanCommand{})

	if err := runner.Run(context.Background(), []string{"/plan"}); err != nil {
		t.Fatalf("Run(/plan) error = %v", err)
	}

	want := "Plan mode is not available in Claude Code Go yet. Enabling plan mode, viewing the current plan, opening the plan file in an editor, and plan-file lifecycle management remain unmigrated.\n"
	if got := buf.String(); got != want {
		t.Fatalf("Run(/plan) output = %q, want %q", got, want)
	}
}
...
func TestRunnerRunTasksCommandReportsFallback(t *testing.T) {
	...
	eng := &recordingEngine{}
	runner := NewRunner(eng, console.NewStreamRenderer(console.NewPrinter(&buf)))
	registerSlashCommands(t, runner, servicecommands.TasksCommand{})

	if err := runner.Run(context.Background(), []string{"/tasks"}); err != nil {
...
// TestRunnerRunDiffCommandReportsFallback verifies /diff is routed through the shared registry and emits the stable guidance fallback.
func TestRunnerRunDiffCommandReportsFallback(t *testing.T) {
	var buf bytes.Buffer
	eng := &recordingEngine{}
	runner := NewRunner(eng, console.NewStreamRenderer(console.NewPrinter(&buf)))
	registerSlashCommands(t, runner, servicecommands.DiffCommand{})

	if err := runner.Run(context.Background(), []string{"/diff"}); err != nil {
		t.Fatalf("Run(/diff) error = %v", err)
	}

	want := "Interactive diff inspection is not available in Claude Code Go yet. Uncommitted change enumeration, per-turn diff views, and diff dialog rendering remain unmigrated.\n"
	if got := buf.String(); got != want {
		t.Fatalf("Run(/diff) output = %q, want %q", got, want)
	}
}
