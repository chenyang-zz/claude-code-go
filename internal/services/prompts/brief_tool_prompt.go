...
// BriefToolPromptSection provides usage guidance for the Brief tool.
type BriefToolPromptSection struct{}

// Name returns the section identifier.
func (s BriefToolPromptSection) Name() string { return "brief_tool_prompt" }

// IsVolatile reports whether this section must be recomputed every turn.
func (s BriefToolPromptSection) IsVolatile() bool { return false }

// Compute generates the Brief tool usage guidance.
func (s BriefToolPromptSection) Compute(ctx context.Context) (string, error) {
	return `# SendUserMessage (Brief)
...
Keep messages tight — the decision, the file:line, the PR number. Second person always ("your config"), never third.`, nil
}
