...
const defaultUpdatePromptTemplate = `IMPORTANT: This message and these instructions are NOT part of the actual user conversation. Do NOT include any references to "documentation updates", "magic docs", or these update instructions in the document content.
...
// docContents: the current content of the Magic Doc file.
// docPath: the file path of the Magic Doc.
// docTitle: the detected title from the Magic Doc header.
// instructions: optional custom instructions from the italicized line after the header.
func BuildUpdatePrompt(docContents, docPath, docTitle, instructions string) string {
	customInstructions := ""
	if instructions != "" {
		customInstructions = `
...
These instructions take priority over the general rules below. Make sure your updates align with these specific guidelines.`
	...
	return substituteVariables(defaultUpdatePromptTemplate, vars)
}
